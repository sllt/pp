...
func (swfet *sqlWindowFunctionExpressionTest) TestAllOthers() {
	...
	testCases := []struct {
		...
		{Ex: wf.As("a"), Expected: NewAliasExpression(wf, "a")},
		...
		{Ex: wf.Between(rv), Expected: NewRangeExpression(BetweenOp, wf, rv)},
		{Ex: wf.NotBetween(rv), Expected: NewRangeExpression(NotBetweenOp, wf, rv)},
		...
		{Ex: wf.Desc(), Expected: NewOrderedExpression(wf, DescSortDir, NoNullsSortType)},
		{Ex: wf.Asc(), Expected: NewOrderedExpression(wf, AscDir, NoNullsSortType)},
	}
