...
func (ces *caseExpressionSuite) TestAs() {
	ce := NewCaseExpression()
	ces.Equal(NewAliasExpression(ce, "a"), ce.As("a"))
}
...
func (ces *caseExpressionSuite) TestAsc() {
	ce := NewCaseExpression()
	ces.Equal(NewOrderedExpression(ce, AscDir, NoNullsSortType), ce.Asc())
}

func (ces *caseExpressionSuite) TestDesc() {
	ce := NewCaseExpression()
	ces.Equal(NewOrderedExpression(ce, DescSortDir, NoNullsSortType), ce.Desc())
}
