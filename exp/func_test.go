...
func (sfes *sqlFunctionExpressionSuite) TestAllOthers() {
	...
	testCases := []struct {
		...
		{Ex: fn.As("a"), Expected: NewAliasExpression(fn, "a")},
		...
		{Ex: fn.Between(rv), Expected: NewRangeExpression(BetweenOp, fn, rv)},
		{Ex: fn.NotBetween(rv), Expected: NewRangeExpression(NotBetweenOp, fn, rv)},
		...
		{Ex: fn.Desc(), Expected: NewOrderedExpression(fn, DescSortDir, NoNullsSortType)},
		{Ex: fn.Asc(), Expected: NewOrderedExpression(fn, AscDir, NoNullsSortType)},
	}
