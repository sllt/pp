...
func (les *literalExpressionSuite) TestAllOthers() {
	...
	testCases := []struct {
		...
		{Ex: le.As("a"), Expected: NewAliasExpression(le, "a")},
		...
		{Ex: le.Asc(), Expected: NewOrderedExpression(le, AscDir, NoNullsSortType)},
		{Ex: le.Desc(), Expected: NewOrderedExpression(le, DescSortDir, NoNullsSortType)},
		{Ex: le.Between(rv), Expected: NewRangeExpression(BetweenOp, le, rv)},
		{Ex: le.NotBetween(rv), Expected: NewRangeExpression(NotBetweenOp, le, rv)},
		...
		{Ex: le.BitwiseInversion(), Expected: NewBitwiseExpression(BitwiseInversionOp, nil, le)},
		{Ex: le.BitwiseOr(bitwiseVals), Expected: NewBitwiseExpression(BitwiseOrOp, le, bitwiseVals)},
		{Ex: le.BitwiseAnd(bitwiseVals), Expected: NewBitwiseExpression(BitwiseAndOp, le, bitwiseVals)},
		{Ex: le.BitwiseXor(bitwiseVals), Expected: NewBitwiseExpression(BitwiseXorOp, le, bitwiseVals)},
		{Ex: le.BitwiseLeftShift(bitwiseVals), Expected: NewBitwiseExpression(BitwiseLeftShiftOp, le, bitwiseVals)},
		{Ex: le.BitwiseRightShift(bitwiseVals), Expected: NewBitwiseExpression(BitwiseRightShiftOp, le, bitwiseVals)},
	}
