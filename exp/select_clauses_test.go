...
func (tse testSQLExpression) Build() (sql string, args []interface{}, err error) {
	return "", nil, nil
}
...
func (scs *selectClausesSuite) TestGroupByAppend() {
	...
	scs.Equal(NewColumnListExpression(g, g2), c2.GroupBy())
}
...
func (scs *selectClausesSuite) TestGroupByAppend_NoPreviousGroupBy() {
	...
	c := NewSelectClauses().GroupByAppend(g)
	c2 := c.GroupByAppend(g2)
