...
func (ces *castExpressionSuite) TestAllOthers() {
	...
	testCases := []struct {
		...
		{Ex: ce.As("a"), Expected: NewAliasExpression(ce, "a")},
		...
		{Ex: ce.Asc(), Expected: NewOrderedExpression(ce, AscDir, NoNullsSortType)},
		{Ex: ce.Desc(), Expected: NewOrderedExpression(ce, DescSortDir, NoNullsSortType)},
		{Ex: ce.Between(rv), Expected: NewRangeExpression(BetweenOp, ce, rv)},
		{Ex: ce.NotBetween(rv), Expected: NewRangeExpression(NotBetweenOp, ce, rv)},
