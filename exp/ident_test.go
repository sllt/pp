...
func (ies *identifierExpressionSuite) TestAs() {
	cases := []struct {
	...
	}{
		{
			...
			Expected: NewAliasExpression(NewIdentifierExpression("", "", "col"), NewIdentifierExpression("", "", "c")),
		},
		{
			Alias:    NewIdentifierExpression("", "table", nil).As("t"),
			Expected: NewAliasExpression(NewIdentifierExpression("", "table", nil), NewIdentifierExpression("", "t", nil)),
		},
		{
		...
		},
		{
			...
			Expected: NewAliasExpression(NewIdentifierExpression("schema", "", nil), NewIdentifierExpression("s", "", nil)),
		},
	}
	for _, tc := range cases {
...
func (ies *identifierExpressionSuite) TestAllOthers() {
	...
	testCases := []struct {
		...
		{Ex: ident.As("a"), Expected: NewAliasExpression(ident, "a")},
		...
		{Ex: ident.Asc(), Expected: NewOrderedExpression(ident, AscDir, NoNullsSortType)},
		{Ex: ident.Desc(), Expected: NewOrderedExpression(ident, DescSortDir, NoNullsSortType)},
		{Ex: ident.Between(rv), Expected: NewRangeExpression(BetweenOp, ident, rv)},
		{Ex: ident.NotBetween(rv), Expected: NewRangeExpression(NotBetweenOp, ident, rv)},
		...
		{Ex: ident.BitwiseInversion(), Expected: NewBitwiseExpression(BitwiseInversionOp, nil, ident)},
		{Ex: ident.BitwiseOr(bitwiseVals), Expected: NewBitwiseExpression(BitwiseOrOp, ident, bitwiseVals)},
		{Ex: ident.BitwiseAnd(bitwiseVals), Expected: NewBitwiseExpression(BitwiseAndOp, ident, bitwiseVals)},
		{Ex: ident.BitwiseXor(bitwiseVals), Expected: NewBitwiseExpression(BitwiseXorOp, ident, bitwiseVals)},
		{Ex: ident.BitwiseLeftShift(bitwiseVals), Expected: NewBitwiseExpression(BitwiseLeftShiftOp, ident, bitwiseVals)},
		{Ex: ident.BitwiseRightShift(bitwiseVals), Expected: NewBitwiseExpression(BitwiseRightShiftOp, ident, bitwiseVals)},
	}
