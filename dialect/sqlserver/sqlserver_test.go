...
type (
	...
	entry struct {
		ID     uint32    `db:"id" pp:"skipinsert,skipupdate"`
		Int    int       `db:"int"`
...
func (sst *sqlserverTest) TestBuild() {
	ds := sst.db.From("entry")
	s, _, err := ds.Select("id", "float", "string", "time", "bool").Build()
	sst.NoError(err)
	sst.Equal("SELECT \"id\", \"float\", \"string\", \"time\", \"bool\" FROM \"entry\"", s)

	s, _, err = ds.Where(pp.C("int").Eq(10)).Build()
	sst.NoError(err)
	sst.Equal("SELECT * FROM \"entry\" WHERE (\"int\" = 10)", s)

	s, args, err := ds.Prepared(true).Where(pp.L("? = ?", pp.C("int"), 10)).Build()
	sst.NoError(err)
...
func (sst *sqlserverTest) TestQuery() {
	...
	sst.NoError(err)
	sst.assertEntries(
		entryTestCase{ds: ds.Order(pp.C("id").Asc()), len: 10, check: func(entry entry, index int) {
		...
		entryTestCase{
			ds:  ds.Where(pp.C("bool").IsTrue()).Order(pp.C("id").Asc()),
			err: "pp: boolean data type is not supported by dialect \"sqlserver\"",
		},
		entryTestCase{ds: ds.Where(pp.C("int").Gt(4)).Order(pp.C("id").Asc()), len: 5, check: func(entry entry, _ int) {
			sst.True(entry.Int > 4)
		}},
		entryTestCase{ds: ds.Where(pp.C("int").Gte(5)).Order(pp.C("id").Asc()), len: 5, check: func(entry entry, _ int) {
			sst.True(entry.Int >= 5)
		}},
		entryTestCase{ds: ds.Where(pp.C("int").Lt(5)).Order(pp.C("id").Asc()), len: 5, check: func(entry entry, _ int) {
			sst.True(entry.Int < 5)
		}},
		entryTestCase{ds: ds.Where(pp.C("int").Lte(4)).Order(pp.C("id").Asc()), len: 5, check: func(entry entry, _ int) {
			sst.True(entry.Int <= 4)
		}},
		entryTestCase{ds: ds.Where(pp.C("int").Between(pp.Range(3, 6))).Order(pp.C("id").Asc()), len: 4, check: func(entry entry, _ int) {
			sst.True(entry.Int >= 3)
			sst.True(entry.Int <= 6)
		}},
		entryTestCase{ds: ds.Where(pp.C("string").Eq("0.100000")).Order(pp.C("id").Asc()), len: 1, check: func(entry entry, _ int) {
			sst.Equal(entry.String, "0.100000")
		}},
		entryTestCase{ds: ds.Where(pp.C("string").Like("0.1%")).Order(pp.C("id").Asc()), len: 1, check: func(entry entry, _ int) {
			sst.Equal(entry.String, "0.100000")
		}},
		entryTestCase{ds: ds.Where(pp.C("string").NotLike("0.1%")).Order(pp.C("id").Asc()), len: 9, check: func(entry entry, _ int) {
			sst.NotEqual(entry.String, "0.100000")
		}},
		entryTestCase{ds: ds.Where(pp.C("string").IsNull()).Order(pp.C("id").Asc()), len: 0, check: func(entry entry, _ int) {
...
func (sst *sqlserverTest) TestQuery_ValueExpressions() {
	...
	sst.NoError(err)
	ds := sst.db.From("entry").Select(pp.Star(), pp.V(true).As("bool_value")).Where(pp.Ex{"int": 1})
	var we wrappedEntry
...
func (sst *sqlserverTest) TestCount() {
	...
	count, err = ds.Where(pp.C("int").Gt(4)).Count()
	sst.NoError(err)
	sst.Equal(int64(5), count)
	count, err = ds.Where(pp.C("int").Gte(4)).Count()
	sst.NoError(err)
	sst.Equal(int64(6), count)
	count, err = ds.Where(pp.C("string").Like("0.1%")).Count()
	sst.NoError(err)
	sst.Equal(int64(1), count)
	count, err = ds.Where(pp.C("string").IsNull()).Count()
	sst.NoError(err)
	sst.Equal(int64(0), count)
}

func (sst *sqlserverTest) TestLimitOffset() {
	ds := sst.db.From("entry").Where(pp.C("id").Gte(1)).Limit(1)
	var e entry
	...
	ds = sst.db.From("entry").Where(pp.C("id").Gte(1)).Order(pp.C("id").Desc()).Limit(1)
	found, err = ds.ScanStruct(&e)
	sst.NoError(err)
	sst.True(found)
	sst.Equal(uint32(10), e.ID)

	ds = sst.db.From("entry").Where(pp.C("id").Gte(1)).Order(pp.C("id").Asc()).Offset(1).Limit(1)
	found, err = ds.ScanStruct(&e)
	sst.NoError(err)
	sst.True(found)
...
func (sst *sqlserverTest) TestLimitOffsetParameterized() {
	ds := sst.db.From("entry").Prepared(true).Where(pp.C("id").Gte(1)).Limit(1)
	var e entry
	found, err := ds.ScanStruct(&e)
	sst.NoError(err)
	sst.True(found)
	...
	ds = sst.db.From("entry").Prepared(true).Where(pp.C("id").Gte(1)).Order(pp.C("id").Desc()).Limit(1)
	found, err = ds.ScanStruct(&e)
	sst.NoError(err)
	sst.True(found)
	sst.Equal(uint32(10), e.ID)

	ds = sst.db.From("entry").Prepared(true).Where(pp.C("id").Gte(1)).Order(pp.C("id").Asc()).Offset(1).Limit(1)
	found, err = ds.ScanStruct(&e)
	sst.NoError(err)
	sst.True(found)
	sst.Equal(uint32(2), e.ID)
...
func (sst *sqlserverTest) TestInsert() {
	ds := sst.db.From("entry")
	now := time.Now()
	_, err := ds.Insert().Rows(pp.Record{
		"Int":    10,
		"Float":  1.00000,
		"String": "1.000000",
		"Time":   now,
		"Bool":   true,
		"Bytes":  pp.Cast(pp.V([]byte("1.000000")), "BINARY(8)"),
	}).Executor().Exec()
	sst.NoError(err)

	var insertedEntry entry
	found, err := ds.Where(pp.C("int").Eq(10)).ScanStruct(&insertedEntry)
	sst.NoError(err)
	sst.True(found)
	...
	entries := []pp.Record{
		{
			...
			"Bool": false, "Bytes": pp.Cast(pp.V([]byte("1.100000")), "BINARY(8)"),
		},
		{
			"Int": 12, "Float": 1.200000, "String": "1.200000", "Time": now,
			"Bool": true, "Bytes": pp.Cast(pp.V([]byte("1.200000")), "BINARY(8)"),
		},
		{
			"Int": 13, "Float": 1.300000, "String": "1.300000", "Time": now,
			"Bool": false, "Bytes": pp.Cast(pp.V([]byte("1.300000")), "BINARY(8)"),
		},
		{
			"Int": 14, "Float": 1.400000, "String": "1.400000", "Time": now,
			"Bool": true, "Bytes": pp.Cast(pp.V([]byte("1.400000")), "BINARY(8)"),
		},
	}
	...
	sst.NoError(ds.Where(pp.C("int").In([]uint32{11, 12, 13, 14})).ScanStructs(&newEntries))
	...
	_, err = ds.Insert().Rows(
		pp.Record{
			...
			"Bool": false, "Bytes": pp.Cast(pp.V([]byte("1.500000")), "BINARY(8)"),
		},
		pp.Record{
			"Int": 16, "Float": 1.600000, "String": "1.600000", "Time": now,
			"Bool": true, "Bytes": pp.Cast(pp.V([]byte("1.600000")), "BINARY(8)"),
		},
		pp.Record{
			"Int": 17, "Float": 1.700000, "String": "1.700000", "Time": now,
			"Bool": false, "Bytes": pp.Cast(pp.V([]byte("1.700000")), "BINARY(8)"),
		},
		pp.Record{
			"Int": 18, "Float": 1.800000, "String": "1.800000", "Time": now,
			"Bool": true, "Bytes": pp.Cast(pp.V([]byte("1.800000")), "BINARY(8)"),
		},
	).Executor().Exec()
	sst.NoError(err)

	newEntries = newEntries[0:0]
	sst.NoError(ds.Where(pp.C("int").In([]uint32{15, 16, 17, 18})).ScanStructs(&newEntries))
	sst.Len(newEntries, 4)
}
...
func (sst *sqlserverTest) TestInsertReturningProducesError() {
	...
	_, err := ds.Insert().Rows(e).Returning(pp.Star()).Executor().ScanStruct(&e)
	sst.Error(err)
}
...
func (sst *sqlserverTest) TestUpdate() {
	ds := sst.db.From("entry")
	var e entry
	found, err := ds.Where(pp.C("int").Eq(9)).Select("id").ScanStruct(&e)
	sst.NoError(err)
	sst.True(found)
	e.Int = 11
	_, err = ds.Where(pp.C("id").Eq(e.ID)).Update().Set(pp.Record{"Int": e.Int}).Executor().Exec()
	sst.NoError(err)

	count, err := ds.Where(pp.C("int").Eq(11)).Count()
	sst.NoError(err)
	sst.Equal(int64(1), count)
}
...
func (sst *sqlserverTest) TestUpdateReturning() {
	ds := sst.db.From("entry")
	var id uint32
	_, err := ds.Where(pp.C("int").Eq(11)).
		Update().
		Set(pp.Record{"int": 9}).
		Returning("id").
		Executor().ScanVal(&id)
	sst.Error(err)
	sst.EqualError(err, "pp: dialect does not support RETURNING clause [dialect=sqlserver]")
}
...
func (sst *sqlserverTest) TestDelete() {
	...
	sst.NoError(err)
	sst.True(found)
	_, err = ds.Where(pp.C("id").Eq(e.ID)).Delete().Executor().Exec()
	sst.NoError(err)
	...
	var id uint32
	found, err = ds.Where(pp.C("id").Eq(e.ID)).ScanVal(&id)
	sst.NoError(err)
	sst.False(found)

	e = entry{}
	found, err = ds.Where(pp.C("int").Eq(8)).Select("id").ScanStruct(&e)
	sst.NoError(err)
	sst.True(found)
	sst.NotEqual(0, e.ID)

	id = 0
	_, err = ds.Where(pp.C("id").Eq(e.ID)).Delete().Returning("id").Executor().ScanVal(&id)
	sst.EqualError(err, "pp: dialect does not support RETURNING clause [dialect=sqlserver]")
}
...
func (sst *sqlserverTest) TestInsertIgnoreNotSupported() {
	...
	entries := []pp.Record{
		{
			...
			"Bytes": pp.Cast(pp.V([]byte("6.100000")), "BINARY(8)"),
		},
		{
			"Int": 9, "Float": 7.200000, "String": "7.200000", "Bool": false, "Time": now,
			"Bytes": pp.Cast(pp.V([]byte("7.200000")), "BINARY(8)"),
		},
		{
	...
	_, err := ds.Insert().Rows(entries).OnConflict(pp.DoNothing()).Executor().Exec()
	sst.Error(err)
