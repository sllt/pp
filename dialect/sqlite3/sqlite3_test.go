...
func (st *sqlite3Suite) TestCompoundQueries() {
	ds1 := st.db.From("entry").Select("int").Where(pp.C("int").Gt(0))
	ds2 := st.db.From("entry").Select("int").Where(pp.C("int").Gt(5))
...
func (st *sqlite3Suite) TestQuery_ValueExpressions() {
	...
	st.NoError(err)
	ds := st.db.From("entry").Select(pp.Star(), pp.V(true).As("bool_value")).Where(pp.Ex{"int": 1})
	var we wrappedEntry
...
func (st *sqlite3Suite) TestInsert() {
	...
	st.NoError(ds.Where(pp.C("int").In([]uint32{11, 12, 13, 14})).ScanStructs(&newEntries))
	...
	st.NoError(ds.Where(pp.C("int").In([]uint32{15, 16, 17, 18})).ScanStructs(&newEntries))
	st.Len(newEntries, 5)
}
...
func (st *sqlite3Suite) TestUpdateReturning() {
	...
	var id uint32
	_, err := ds.
		Where(pp.C("int").Eq(11)).
		Update().
		Set(map[string]interface{}{"int": 9}).
		Returning("id").
		Executor().ScanVal(&id)
	st.Error(err)
	st.EqualError(err, "pp: dialect does not support RETURNING clause [dialect=sqlite3]")
}
...
func (st *sqlite3Suite) TestInsert_OnConflict() {
	...
	_, err = ds.Where(pp.C("id").Eq(11)).ScanStruct(&entryActual)
	st.NoError(err)
	...
	).OnConflict(pp.DoNothing()).Executor().Exec()
	st.NoError(err)
	...
	).OnConflict(pp.DoUpdate("id", pp.Record{"string": "upsert"})).Executor().Exec()
	st.NoError(err)
	...
	_, err = ds.Insert().
		Rows(entries).
		OnConflict(pp.DoUpdate("id", pp.Record{"string": "upsert"}).Where(pp.C("id").Eq(9))).
		Executor().Exec()
	st.EqualError(err, "pp: dialect does not support upsert with where clause [dialect=sqlite3]")
}
