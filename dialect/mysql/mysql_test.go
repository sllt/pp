...
const defaultDBURI = "root@/ppmysql?parseTime=true"
...
func (mt *mysqlTest) TestBuild() {
	ds := mt.db.From("entry")
	mt.assertSQL(
		sqlTestCase{ds: ds.Select("id", "float", "string", "time", "bool"), sql: "SELECT `id`, `float`, `string`, `time`, `bool` FROM `entry`"},
		sqlTestCase{ds: ds.Where(pp.C("int").Eq(10)), sql: "SELECT * FROM `entry` WHERE (`int` = 10)"},
		sqlTestCase{
			ds:  ds.Prepared(true).Where(pp.L("? = ?", pp.C("int"), 10)),
...
func (mt *mysqlTest) TestQuery() {
	...
	mt.assertEntries(
		...
		entryTestCase{ds: ds.Where(pp.C("bool").IsTrue()).Order(pp.C("id").Asc()), len: 5, check: func(entry entry, _ int) {
			mt.True(entry.Bool)
		}},
...
func (mt *mysqlTest) TestQuery_ValueExpressions() {
	...
	mt.NoError(err)
	ds := mt.db.From("entry").Select(pp.Star(), pp.V(true).As("bool_value")).Where(pp.Ex{"int": 1})
	var we wrappedEntry
...
func (mt *mysqlTest) TestInsert() {
	...
	mt.NoError(ds.Where(pp.C("int").In([]uint32{11, 12, 13, 14})).ScanStructs(&newEntries))
	mt.Len(newEntries, 4)
	...
	mt.NoError(ds.Where(pp.C("int").In([]uint32{15, 16, 17, 18})).ScanStructs(&newEntries))
	mt.Len(newEntries, 4)
}
...
func (mt *mysqlTest) TestUpdate() {
	...
	mt.True(found)
	e.Int = 11
	_, err = ds.Where(pp.C("id").Eq(e.ID)).Update().Set(e).Executor().Exec()
	mt.NoError(err)
...
func (mt *mysqlTest) TestUpdateReturning() {
	...
	_, err := ds.Where(pp.C("int").Eq(11)).
		...
		Executor().ScanVal(&id)
	mt.Error(err)
	mt.EqualError(err, "pp: dialect does not support RETURNING clause [dialect=mysql]")
}
...
func (mt *mysqlTest) TestInsert_OnConflict() {
	...
	_, err := ds.Insert().Rows(e).OnConflict(pp.DoNothing()).Executor().Exec()
	mt.NoError(err)
	...
	_, err = ds.Insert().
		Rows(e2).
		OnConflict(pp.DoUpdate("int", pp.Record{"string": "upsert"})).
		Executor().Exec()
	mt.NoError(err)
	_, err = ds.Where(pp.C("int").Eq(10)).ScanStruct(&entryActual)
	mt.NoError(err)
	...
	_, err = ds.Insert().
		Rows(entries).
		OnConflict(pp.DoUpdate("int", pp.Record{"string": "upsert"}).Where(pp.C("int").Eq(9))).
		Executor().Exec()
	mt.EqualError(err, "pp: dialect does not support upsert with where clause [dialect=mysql]")
}

func (mt *mysqlTest) TestWindowFunction() {
	var version string
	ok, err := mt.db.Select(pp.Func("version")).ScanVal(&version)
	mt.NoError(err)
	mt.True(ok)
	...
	ds := mt.db.From("entry").
		Select("int", pp.ROW_NUMBER().OverName(pp.I("w")).As("id")).
		Window(pp.W("w").OrderBy(pp.I("int").Desc()))
	...
	mt.Error(ds.WithDialect("mysql").ScanStructs(&entries), "pp: adapter does not support window function clause")
}
...
func (mt *mysqlTest) TestInsertFromSelect() {
	...
	subquery := pp.Select(
		...
		pp.C("bool"),
		pp.C("bytes"),
	).From(pp.T("entry")).Where(pp.C("int").Eq(9))
