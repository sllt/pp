...
func (pt *postgresTest) TestBuild() {
	ds := pt.db.From("entry")
...
func (pt *postgresTest) TestQuery_ValueExpressions() {
	...
	pt.NoError(err)
	ds := pt.db.From("entry").Select(pp.Star(), pp.V(true).As("bool_value")).Where(pp.Ex{"int": 1})
	var we wrappedEntry
...
func (pt *postgresTest) TestInsert() {
	...
	pt.NoError(ds.Where(pp.C("int").In([]uint32{11, 12, 13, 14})).ScanStructs(&newEntries))
	pt.Len(newEntries, 4)
	...
	pt.NoError(ds.Where(pp.C("int").In([]uint32{15, 16, 17, 18})).ScanStructs(&newEntries))
	pt.Len(newEntries, 4)
}
...
func (pt *postgresTest) TestInsertReturning() {
	...
	now := time.Now()
	e := entry{Int: 10, Float: 1.000000, String: "1.000000", Time: now, Bool: true, Bytes: []byte("1.000000")}
	found, err := ds.Insert().Rows(e).Returning(pp.Star()).Executor().ScanStruct(&e)
	pt.NoError(err)
	pt.True(found)
	pt.True(e.ID > 0)
...
func (pt *postgresTest) TestUpdate() {
	...
	var id uint32
	found, err = ds.Where(pp.C("int").Eq(11)).
		Update().
		Set(pp.Record{"int": 9}).
...
func (pt *postgresTest) TestInsert_OnConflict() {
	...
	_, err = ds.Insert().
		...
		Executor().Exec()
	pt.NoError(err)
	_, err = ds.Where(pp.C("int").Eq(0)).ScanStruct(&entryActual)
	pt.NoError(err)
	...
	_, err = ds.Insert().
		Rows(entries).
		OnConflict(pp.DoUpdate("int", pp.Record{"string": "upsert"}).Where(pp.I("excluded.int").Eq(2))).
		Executor().
		Exec()
	pt.NoError(err)

	var entry8, entry9 entry
	_, err = ds.Where(pp.Ex{"int": 1}).ScanStruct(&entry8)
	pt.NoError(err)
	pt.Equal("0.100000", entry8.String)

	_, err = ds.Where(pp.Ex{"int": 2}).ScanStruct(&entry9)
	pt.NoError(err)
...
func (pt *postgresTest) TestOrderByFunction() {
	ds := pt.db.From("entry").
		Select(pp.ROW_NUMBER().Over(pp.W()).As("id")).Window().Order(pp.ROW_NUMBER().Over(pp.W()).Desc())
