...
func (ds *databaseSuite) TestLogger() {
	...
	db.Trace("TEST", "")
	ds.Equal([]string{
		"[pp] QUERY [query:=`SELECT * FROM \"items\"`]",
		"[pp] EXEC [query:=`SELECT * FROM \"items\" WHERE \"id\" = ?` args:=[1]]",
		"[pp] TEST",
	}, logger.Messages)
}
...
func (ds *databaseSuite) TestScanStruct() {
	...
	ds.EqualError(err, "pp: type must be a pointer to a struct when scanning into a struct")
	_, err = db.ScanStruct([]testActionItem{}, `SELECT * FROM "items" LIMIT 1`)
	ds.EqualError(err, "pp: type must be a pointer to a struct when scanning into a struct")
	_, err = db.ScanStruct(&item, `SELECT "test" FROM "items" LIMIT 1`)
	ds.EqualError(err, `pp: unable to find corresponding field to column "test" returned by query`)
}
...
func (ds *databaseSuite) TestScanVal() {
	...
	ds.False(found)
	ds.EqualError(err, "pp: type must be a pointer when scanning into val")
...
func (ds *databaseSuite) TestExec() {
	...
	ds.NoError(err)
	_, err = db.Exec(`UPDATE "items" SET "address"='111 Test Addr',"name"='Test1' WHERE ("name" IS NULL)`)
	ds.EqualError(err, "pp: mock error")
}
...
func (ds *databaseSuite) TestQueryRow() {
	...
	rows = db.QueryRow(`SELECT * FROM "items"`)
	ds.EqualError(rows.Scan(&address, &name), "pp: mock error")
}
...
func (ds *databaseSuite) TestBegin() {
	...
	_, err = db.Begin()
	ds.EqualError(err, "pp: transaction error")
}
...
func (ds *databaseSuite) TestWithTx() {
	...
	cases := []struct {
	...
	}{
		{
			...
			f:       func(_ *pp.TxDatabase) error { return nil },
			wantErr: false,
		},
		{
			expectf: func(mock sqlmock.Sqlmock) {
			...
			f:       func(_ *pp.TxDatabase) error { return nil },
			wantErr: true,
			errStr:  "pp: transaction begin error",
		},
		{
			expectf: func(mock sqlmock.Sqlmock) {
				mock.ExpectBegin()
			...
			f:       func(_ *pp.TxDatabase) error { return errors.New("transaction error") },
			wantErr: true,
			errStr:  "pp: transaction error",
		},
		{
			expectf: func(mock sqlmock.Sqlmock) {
				mock.ExpectBegin()
			...
			f:       func(_ *pp.TxDatabase) error { return errors.New("something wrong") },
			wantErr: true,
			errStr:  "pp: transaction rollback error",
		},
		{
			expectf: func(mock sqlmock.Sqlmock) {
				mock.ExpectBegin()
...
func (ds *databaseSuite) TestRollbackOnPanic() {
	...
	db := pp.New("mock", mDB)
	_ = db.WithTx(func(_ *pp.TxDatabase) error {
...
func (tds *txdatabaseSuite) TestLogger() {
	...
	tx, err := pp.New("db-mock", mDB).Begin()
	tds.NoError(err)
	logger := new(dbTestMockLogger)
	tx.Logger(logger)
	var items []testActionItem
	...
	tds.Equal([]string{
		"[pp - transaction] QUERY [query:=`SELECT * FROM \"items\"`] ",
		"[pp - transaction] EXEC [query:=`SELECT * FROM \"items\" WHERE \"id\" = ?` args:=[1]] ",
		"[pp - transaction] COMMIT",
	}, logger.Messages)
}
...
func (tds *txdatabaseSuite) TestScanStruct() {
	...
	tds.EqualError(err, "pp: type must be a pointer to a struct when scanning into a struct")
	_, err = tx.ScanStruct([]testActionItem{}, `SELECT * FROM "items" LIMIT 1`)
	tds.EqualError(err, "pp: type must be a pointer to a struct when scanning into a struct")
	_, err = tx.ScanStruct(&item, `SELECT "test" FROM "items" LIMIT 1`)
	tds.EqualError(err, `pp: unable to find corresponding field to column "test" returned by query`)
	tds.NoError(tx.Commit())
}
...
func (tds *txdatabaseSuite) TestScanVal() {
	...
	tds.False(found)
	tds.EqualError(err, "pp: type must be a pointer when scanning into val")
...
func (tds *txdatabaseSuite) TestExec() {
	...
	tds.NoError(err)
	_, err = tx.Exec(`UPDATE "items" SET "address"='111 Test Addr',"name"='Test1' WHERE ("name" IS NULL)`)
	tds.EqualError(err, "pp: mock error")
	tds.NoError(tx.Commit())
}
...
func (tds *txdatabaseSuite) TestQueryRow() {
	...
	rows = tx.QueryRow(`SELECT * FROM "items"`)
	tds.EqualError(rows.Scan(&address, &name), "pp: mock error")
	tds.NoError(tx.Commit())
}
