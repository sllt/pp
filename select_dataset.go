...
var ErrQueryFactoryNotFoundError = errors.New(
	"unable to execute query did you use pp.Database#From to create the dataset",
)
...
func (sd *SelectDataset) Build() (sql string, params []interface{}, err error) {
	return sd.selectSQLBuilder().Build()
}
...
func (sd *SelectDataset) AppendSQL(b builder.SQLBuilder) {
	if sd.err != nil {
		b.SetError(sd.err)
		return
	}
...
func (sd *SelectDataset) selectSQLBuilder() builder.SQLBuilder {
	buf := builder.NewSQLBuilder(sd.isPrepared.Bool())
	if sd.err != nil {
