...
// Insert rows. Rows can be a map, pp.Record or struct. See examples.
...
// or as part of Build. This can be used by end users to record errors while building up queries without having to
...
// specifying a pp tag with `skipinsert`
//
//	type Item struct{
...
func (id *InsertDataset) Build() (sql string, params []interface{}, err error) {
	return id.insertSQLBuilder().Build()
}

// Appends this Dataset's INSERT statement to the SQLBuilder
// This is used internally when using inserts in CTEs
func (id *InsertDataset) AppendSQL(b builder.SQLBuilder) {
	if id.err != nil {
		b.SetError(id.err)
		return
	}
...
func (id *InsertDataset) insertSQLBuilder() builder.SQLBuilder {
	buf := builder.NewSQLBuilder(id.isPrepared.Bool())
	if id.err != nil {
