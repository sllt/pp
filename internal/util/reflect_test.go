...
func (rt *reflectTest) TestGetColumnMap_withStructPpTags() {
	type TestStruct struct {
		Str    string `pp:"skipinsert,skipupdate"`
		Int    int64  `pp:"skipinsert"`
		Bool   bool   `pp:"skipupdate"`
		Empty  bool   `pp:"defaultifempty"`
		Valuer *sql.NullString
	}
	var ts TestStruct
	cm, err := util.GetColumnMap(&ts)
...
func (rt *reflectTest) TestGetColumnMap_withStructWithTagAndPpTag() {
	type TestStruct struct {
		Str    string          `db:"s" pp:"skipinsert,skipupdate"`
		Int    int64           `db:"i" pp:"skipinsert"`
		Bool   bool            `db:"b" pp:"skipupdate"`
		Valuer *sql.NullString `db:"v"`
	}
	var ts TestStruct
	cm, err := util.GetColumnMap(&ts)
...
func (rt *reflectTest) TestGetColumnMap_withNonStruct() {
	...
	rt.EqualError(err, "pp: cannot scan into this type: int64")
}
