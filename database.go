...
// can be found at https://github.com/sllt/pp/tree/master/adapters
//
...
//
//	func main() {
//	    sqlDb, err := sql.Open("postgres", "user=postgres dbname=pppostgres sslmode=disable ")
//	    if err != nil {
//	        panic(err.Error())
//	    }
//	    db := pp.New("postgres", sqlDb)
//	}
//
...
//
//	var ids []uint32
//	if err := db.From("items").Where(pp.I("id").Gt(10)).Pluck("id", &ids); err != nil {
//	    panic(err.Error())
//	}
//	fmt.Printf("%+v", ids)
...
//	fmt.Printf("%+v", ids)
//
// from...: Sources for you dataset, could be table names (strings), a pp.Literal or another pp.Dataset
...
func (d *Database) Trace(op, sqlString string, args ...interface{}) {
	if d.logger != nil {
		if sqlString != "" {
			if len(args) != 0 {
				d.logger.Printf("[pp] %s [query:=`%s` args:=%+v]", op, sqlString, args)
			} else {
				d.logger.Printf("[pp] %s [query:=`%s`]", op, sqlString)
			}
		} else {
			d.logger.Printf("[pp] %s", op)
		}
	}
}
...
//
//	sql, args, err := db.From("items").Where(pp.I("id").Gt(10)).Build(true)
//	if err != nil{
...
// You can use this in tandem with a dataset by doing the following.
//
//	sql, err := db.From("items").Where(pp.I("id").Gt(10)).Build()
//	if err != nil{
//	    panic(err.Error()) //you could gracefully handle the error also
//	}
//	rows, err := stmt.Query(args)
...
// You can use this in tandem with a dataset by doing the following.
//
//	sql, err := db.From("items").Where(pp.I("id").Gt(10)).Limit(1).Build()
//	if err != nil{
//	    panic(err.Error()) //you could gracefully handle the error also
//	}
...
func (td *TxDatabase) Trace(op, sqlString string, args ...interface{}) {
	if td.logger != nil {
		if sqlString != "" {
			if len(args) != 0 {
				td.logger.Printf("[pp - transaction] %s [query:=`%s` args:=%+v] ", op, sqlString, args)
			} else {
				td.logger.Printf("[pp - transaction] %s [query:=`%s`] ", op, sqlString)
			}
		} else {
			td.logger.Printf("[pp - transaction] %s", op)
		}
	}
}
