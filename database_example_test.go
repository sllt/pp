...
func ExampleDatabase_Exec() {
	...
	_, err := db.Exec(`DROP TABLE "user_role"; DROP TABLE "pp_user"`)
	if err != nil {
		fmt.Println("Error occurred while dropping tables", err.Error())
	}
	fmt.Println("Dropped tables user_role and pp_user")
	// Output:
	// Dropped tables user_role and pp_user
}
...
func ExampleDatabase_ExecContext() {
	...
	defer cancel()
	_, err := db.ExecContext(ctx, `DROP TABLE "user_role"; DROP TABLE "pp_user"`)
	if err != nil {
		fmt.Println("Error occurred while dropping tables", err.Error())
	}
	fmt.Println("Dropped tables user_role and pp_user")
...
func ExampleDatabase_From() {
	...
	if err := db.From("pp_user").Select("first_name").ScanVals(&names); err != nil {
