...
type (
	...
	DeleteSQLGenerator interface {
		Dialect() string
		Generate(b builder.SQLBuilder, clauses exp.DeleteClauses)
	}
...
func (dsg *deleteSQLGenerator) Generate(b builder.SQLBuilder, clauses exp.DeleteClauses) {
	if !clauses.HasFrom() {
...
func (dsg *deleteSQLGenerator) DeleteBeginSQL(b builder.SQLBuilder, from exp.ColumnListExpression, multiTable bool) {
