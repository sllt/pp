...
type (
	commonSQLTestCase struct {
		gen        func(builder builder.SQLBuilder)
		sql        string
		isPrepared bool
		err        string
		args       []interface{}
...
func (csgs *commonSQLGeneratorSuite) assertCases(testCases ...commonSQLTestCase) {
	for _, tc := range testCases {
		b := builder.NewSQLBuilder(tc.isPrepared)
		tc.gen(b)
		switch {
		case len(tc.err) > 0:
...
func (csgs *commonSQLGeneratorSuite) TestReturningSQL() {
	returningGen := func(csgs CommonSQLGenerator) func(builder.SQLBuilder) {
		return func(sb builder.SQLBuilder) {
			csgs.ReturningSQL(sb, exp.NewColumnListExpression("a", "b"))
		}
	}

	returningNoColsGen := func(csgs CommonSQLGenerator) func(builder.SQLBuilder) {
		return func(sb builder.SQLBuilder) {
			csgs.ReturningSQL(sb, exp.NewColumnListExpression())
		}
	}

	returningNilExpGen := func(csgs CommonSQLGenerator) func(builder.SQLBuilder) {
		return func(sb builder.SQLBuilder) {
	...
	csgs.assertCases(
		...
		commonSQLTestCase{gen: returningGen(csgs2), err: `pp: dialect does not support RETURNING clause [dialect=test]`},
		commonSQLTestCase{gen: returningGen(csgs2), err: `pp: dialect does not support RETURNING clause [dialect=test]`},
	)
}

func (csgs *commonSQLGeneratorSuite) TestFromSQL() {
	fromGen := func(csgs CommonSQLGenerator) func(builder.SQLBuilder) {
		return func(sb builder.SQLBuilder) {
			csgs.FromSQL(sb, exp.NewColumnListExpression("a", "b"))
		}
	}

	fromNoColsGen := func(csgs CommonSQLGenerator) func(builder.SQLBuilder) {
		return func(sb builder.SQLBuilder) {
			csgs.FromSQL(sb, exp.NewColumnListExpression())
		}
	}

	fromNilExpGen := func(csgs CommonSQLGenerator) func(builder.SQLBuilder) {
		return func(sb builder.SQLBuilder) {
			csgs.FromSQL(sb, nil)
		}
	}
...
func (csgs *commonSQLGeneratorSuite) TestWhereSQL() {
	whereAndGen := func(csgs CommonSQLGenerator, exps ...exp.Expression) func(builder.SQLBuilder) {
		return func(sb builder.SQLBuilder) {
			csgs.WhereSQL(sb, exp.NewExpressionList(exp.AndType, exps...))
		}
	}

	whereOrGen := func(csgs CommonSQLGenerator, exps ...exp.Expression) func(builder.SQLBuilder) {
		return func(sb builder.SQLBuilder) {
...
func (csgs *commonSQLGeneratorSuite) TestOrderSQL() {
	orderGen := func(csgs CommonSQLGenerator, o ...exp.OrderedExpression) func(builder.SQLBuilder) {
		return func(sb builder.SQLBuilder) {
...
func (csgs *commonSQLGeneratorSuite) TestLimitSQL() {
	limitGen := func(csgs CommonSQLGenerator, l interface{}) func(builder.SQLBuilder) {
		return func(sb builder.SQLBuilder) {
			csgs.LimitSQL(sb, l)
		}
	}
...
func (csgs *commonSQLGeneratorSuite) TestUpdateExpressionSQL() {
	updateGen := func(csgs CommonSQLGenerator, ues ...exp.UpdateExpression) func(builder.SQLBuilder) {
		return func(sb builder.SQLBuilder) {
