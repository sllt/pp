...
func (tae *testAppendableExpression) AppendSQL(b builder.SQLBuilder) {
	if tae.err != nil {
		b.SetError(tae.err)
		return
	}
...
func (esgs *expressionSQLGeneratorSuite) assertCases(esg ExpressionSQLGenerator, cases ...expressionTestCase) {
	for i, c := range cases {
		b := builder.NewSQLBuilder(c.isPrepared)
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_ErroredBuilder() {
	...
	b := builder.NewSQLBuilder(false).SetError(expectedErr)
	esg.Generate(b, 1)
	...
	b = builder.NewSQLBuilder(true).SetError(err)
	esg.Generate(b, true)
	sql, args, err = b.Build()
	esgs.Equal(expectedErr, err)
	esgs.Empty(sql)
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_UnsupportedType() {
	...
	esgs.assertCases(
		NewExpressionSQLGenerator("test", DefaultDialectOptions()),
		expressionTestCase{val: strct{}, err: "pp_encode_error: Unable to encode value {}"},
		expressionTestCase{val: strct{}, err: "pp_encode_error: Unable to encode value {}", isPrepared: true},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_Valuer() {
	...
	esgs.assertCases(
		...
		expressionTestCase{val: datasetValuerType{err: err}, err: "pp: valuer error"},
		expressionTestCase{
			val: datasetValuerType{err: err}, isPrepared: true, err: "pp: valuer error",
		},
		expressionTestCase{
			val: val, sql: "NULL",
		},
...
func (esgs *expressionSQLGeneratorSuite) TestGenerateUnsupportedExpression() {
	errMsg := "pp: unsupported expression type sqlgen_test.unknownExpression"
	esgs.assertCases(
		NewExpressionSQLGenerator("test", DefaultDialectOptions()),
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_AppendableExpression() {
	...
	esgs.assertCases(
		...
		expressionTestCase{val: ae, err: "pp: expected error"},
		expressionTestCase{val: ae, err: "pp: expected error", isPrepared: true},
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_BooleanExpression() {
	...
	esgs.assertCases(
		NewExpressionSQLGenerator("test", opts),
		expressionTestCase{val: ident.Eq(1), err: "pp: boolean operator 'eq' not supported"},
		expressionTestCase{val: ident.Neq(1), err: "pp: boolean operator 'neq' not supported"},
		expressionTestCase{val: ident.Is(true), err: "pp: boolean operator 'is' not supported"},
		expressionTestCase{val: ident.IsNot(true), err: "pp: boolean operator 'isnot' not supported"},
		expressionTestCase{val: ident.Gt(1), err: "pp: boolean operator 'gt' not supported"},
		expressionTestCase{val: ident.Gte(1), err: "pp: boolean operator 'gte' not supported"},
		expressionTestCase{val: ident.Lt(1), err: "pp: boolean operator 'lt' not supported"},
		expressionTestCase{val: ident.Lte(1), err: "pp: boolean operator 'lte' not supported"},
		expressionTestCase{val: ident.In([]int64{1, 2, 3}), err: "pp: boolean operator 'in' not supported"},
		expressionTestCase{val: ident.NotIn([]int64{1, 2, 3}), err: "pp: boolean operator 'notin' not supported"},
		expressionTestCase{val: ident.Like("a%"), err: "pp: boolean operator 'like' not supported"},
		expressionTestCase{val: ident.Like(re), err: "pp: boolean operator 'regexplike' not supported"},
		expressionTestCase{val: ident.ILike("a%"), err: "pp: boolean operator 'ilike' not supported"},
		expressionTestCase{val: ident.ILike(re), err: "pp: boolean operator 'regexpilike' not supported"},
		expressionTestCase{val: ident.NotLike("a%"), err: "pp: boolean operator 'notlike' not supported"},
		expressionTestCase{val: ident.NotLike(re), err: "pp: boolean operator 'regexpnotlike' not supported"},
		expressionTestCase{val: ident.NotILike("a%"), err: "pp: boolean operator 'notilike' not supported"},
		expressionTestCase{val: ident.NotILike(re), err: "pp: boolean operator 'regexpnotilike' not supported"},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_BitwiseExpression() {
	...
	esgs.assertCases(
		NewExpressionSQLGenerator("test", opts),
		expressionTestCase{val: ident.BitwiseInversion(), err: "pp: bitwise operator 'Inversion' not supported"},
		expressionTestCase{val: ident.BitwiseAnd(1), err: "pp: bitwise operator 'AND' not supported"},
		expressionTestCase{val: ident.BitwiseOr(1), err: "pp: bitwise operator 'OR' not supported"},
		expressionTestCase{val: ident.BitwiseXor(1), err: "pp: bitwise operator 'XOR' not supported"},
		expressionTestCase{val: ident.BitwiseLeftShift(1), err: "pp: bitwise operator 'Left Shift' not supported"},
		expressionTestCase{val: ident.BitwiseRightShift(1), err: "pp: bitwise operator 'Right Shift' not supported"},
	)
}
func (esgs *expressionSQLGeneratorSuite) TestGenerate_RangeExpression() {
	...
	esgs.assertCases(
		NewExpressionSQLGenerator("test", opts),
		expressionTestCase{val: betweenNum, err: "pp: range operator between not supported"},
		expressionTestCase{val: betweenNum, err: "pp: range operator between not supported"},

		expressionTestCase{val: notBetweenNum, err: "pp: range operator not between not supported"},
		expressionTestCase{val: notBetweenNum, err: "pp: range operator not between not supported"},

		expressionTestCase{val: betweenStr, err: "pp: range operator between not supported"},
		expressionTestCase{val: betweenStr, err: "pp: range operator between not supported"},

		expressionTestCase{val: notBetweenStr, err: "pp: range operator not between not supported"},
		expressionTestCase{val: notBetweenStr, err: "pp: range operator not between not supported"},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_CommonTableExpressionSlice() {
	...
	esgs.assertCases(
		NewExpressionSQLGenerator("test", opts),
		expressionTestCase{val: cteNoArgs, err: "pp: dialect does not support CTE WITH clause [dialect=test]"},
		expressionTestCase{val: cteNoArgs, err: "pp: dialect does not support CTE WITH clause [dialect=test]", isPrepared: true},

		expressionTestCase{val: cteArgs, err: "pp: dialect does not support CTE WITH clause [dialect=test]"},
		expressionTestCase{val: cteArgs, err: "pp: dialect does not support CTE WITH clause [dialect=test]", isPrepared: true},

		expressionTestCase{val: cteRecursiveNoArgs, err: "pp: dialect does not support CTE WITH clause [dialect=test]"},
		expressionTestCase{val: cteRecursiveNoArgs, err: "pp: dialect does not support CTE WITH clause [dialect=test]", isPrepared: true},

		expressionTestCase{val: cteRecursiveArgs, err: "pp: dialect does not support CTE WITH clause [dialect=test]"},
		expressionTestCase{val: cteRecursiveArgs, err: "pp: dialect does not support CTE WITH clause [dialect=test]", isPrepared: true},
	)
	opts = DefaultDialectOptions()
	...
	esgs.assertCases(
		...
		expressionTestCase{
			...
			err: "pp: dialect does not support CTE WITH RECURSIVE clause [dialect=test]",
		},
		expressionTestCase{
			val:        cteRecursiveNoArgs,
			err:        "pp: dialect does not support CTE WITH RECURSIVE clause [dialect=test]",
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_IdentifierExpression() {
	...
	esgs.assertCases(
		...
		expressionTestCase{
			...
			err: `pp: a empty identifier was encountered, please specify a "schema", "table" or "column"`,
		},
		expressionTestCase{
		...
		expressionTestCase{
			...
			err: `pp: unexpected col type must be string or LiteralExpression received bool`,
		},
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_LateralExpression() {
	...
	esgs.assertCases(
		NewExpressionSQLGenerator("test", do),
		expressionTestCase{val: lateralExp, err: "pp: dialect does not support lateral expressions [dialect=test]"},
		expressionTestCase{val: lateralExp, err: "pp: dialect does not support lateral expressions [dialect=test]", isPrepared: true},
	)
}
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_CaseExpression() {
	...
	esgs.assertCases(
		...
		expressionTestCase{
			...
			err: "pp: when conditions not found for case statement",
		},
	)
...
func (esgs *expressionSQLGeneratorSuite) TestGenerate_ExpressionMapWithABadOp() {
	esgs.assertCases(
		...
		expressionTestCase{
			...
			err: "pp: unsupported expression type badOp",
		},
		expressionTestCase{
			val:        exp.Ex{"a": exp.Op{"badOp": true}},
			isPrepared: true,
