...
func (igs *insertSQLGeneratorSuite) TestGenerate_UnsupportedFragment() {
	...
	d.Generate(b, ic)
	igs.assertErrorSQL(b, `pp: unsupported INSERT SQL fragment UpdateBeginSQLFragment`)
}
...
func (igs *insertSQLGeneratorSuite) TestGenerate_colsAndVals() {
	...
	igs.assertCases(
		...
		insertTestCase{clause: bic, err: `pp: rows with different value length expected 1 got 2`},
		insertTestCase{clause: bic, err: `pp: rows with different value length expected 1 got 2`, isPrepared: true},
	)
}
...
func (igs *insertSQLGeneratorSuite) TestGenerate_withNoInto() {
	...
	expectedErr := "pp: no source found when generating insert sql"
	igs.assertCases(
		NewInsertSQLGenerator("test", opts),
...
func (igs *insertSQLGeneratorSuite) TestGenerate_onConflict() {
	...
	igs.assertCases(
		...
		insertTestCase{clause: icDuBad, err: "pp: unsupported update interface type bool"},
		insertTestCase{clause: icDuBad, err: "pp: unsupported update interface type bool", isPrepared: true},
	)
	...
	expectedErr := "pp: dialect does not support upsert with where clause [dialect=test]"
	igs.assertCases(
		NewInsertSQLGenerator("test", opts),
...
func (igs *insertSQLGeneratorSuite) TestGenerate_withCommonTables() {
	...
	expectedErr := "pp: dialect does not support CTE WITH clause [dialect=test]"
	igs.assertCases(
		NewInsertSQLGenerator("test", opts),
	...
	expectedErr = "pp: dialect does not support CTE WITH RECURSIVE clause [dialect=test]"
	igs.assertCases(
		NewInsertSQLGenerator("test", opts),
		insertTestCase{
			clause: icCte1,
