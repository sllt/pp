...
type (
	...
	SelectSQLGenerator interface {
		Dialect() string
		Generate(b builder.SQLBuilder, clauses exp.SelectClauses)
	}
...
func (ssg *selectSQLGenerator) Generate(b builder.SQLBuilder, clauses exp.SelectClauses) {
...
func (ssg *selectSQLGenerator) selectSQLCommon(b builder.SQLBuilder, clauses exp.SelectClauses) {
	dc := clauses.Distinct()
	if dc != nil {
...
func (ssg *selectSQLGenerator) SelectSQL(b builder.SQLBuilder, clauses exp.SelectClauses) {
...
func (ssg *selectSQLGenerator) SelectWithLimitSQL(b builder.SQLBuilder, clauses exp.SelectClauses) {
	b.Write(ssg.DialectOptions().SelectClause).WriteRunes(ssg.DialectOptions().SpaceRune)
...
func (ssg *selectSQLGenerator) JoinSQL(b builder.SQLBuilder, joins exp.JoinExpressions) {
	if len(joins) > 0 {
		for _, j := range joins {
...
func (ssg *selectSQLGenerator) GroupBySQL(b builder.SQLBuilder, groupBy exp.ColumnListExpression) {
...
func (ssg *selectSQLGenerator) HavingSQL(b builder.SQLBuilder, having exp.ExpressionList) {
...
func (ssg *selectSQLGenerator) OffsetSQL(b builder.SQLBuilder, offset uint) {
	if offset > 0 {
...
func (ssg *selectSQLGenerator) CompoundsSQL(b builder.SQLBuilder, compounds []exp.CompoundExpression) {
...
func (ssg *selectSQLGenerator) ForSQL(b builder.SQLBuilder, lockingClause exp.Lock) {
...
func (ssg *selectSQLGenerator) WindowSQL(b builder.SQLBuilder, windows []exp.WindowExpression) {
	weLen := len(windows)
	if weLen == 0 {
		return
	}
...
func (ssg *selectSQLGenerator) joinConditionSQL(b builder.SQLBuilder, jc exp.JoinCondition) {
	switch t := jc.(type) {
...
func (ssg *selectSQLGenerator) joinUsingConditionSQL(b builder.SQLBuilder, jc exp.JoinUsingCondition) {
...
func (ssg *selectSQLGenerator) joinOnConditionSQL(b builder.SQLBuilder, jc exp.JoinOnCondition) {
