...
func (dsgs *deleteSQLGeneratorSuite) TestGenerate_withUnsupportedFragment() {
	...
	dsgs.assertCases(
		...
		deleteTestCase{clause: dc, err: `pp: unsupported DELETE SQL fragment InsertBeingSQLFragment`},
		deleteTestCase{clause: dc, err: `pp: unsupported DELETE SQL fragment InsertBeingSQLFragment`, isPrepared: true},
	)
}
...
func (dsgs *deleteSQLGeneratorSuite) TestGenerate_withErroredBuilder() {
	...
	b := builder.NewSQLBuilder(false).SetError(errors.New("expected error"))
	d.Generate(b, dc)
	dsgs.assertErrorSQL(b, "pp: expected error")

	b = builder.NewSQLBuilder(true).SetError(errors.New("expected error"))
	d.Generate(b, dc)
	dsgs.assertErrorSQL(b, "pp: expected error")
}
...
func (dsgs *deleteSQLGeneratorSuite) TestGenerate_withReturning() {
	...
	expectedErr := `pp: dialect does not support RETURNING clause [dialect=test]`
	dsgs.assertCases(
		NewDeleteSQLGenerator("test", opts),
