...
func (ssgs *selectSQLGeneratorSuite) TestGenerate_UnsupportedFragment() {
	...
	sc := exp.NewSelectClauses().SetFrom(exp.NewColumnListExpression("test"))
	expectedErr := "pp: unsupported SELECT SQL fragment InsertBeingSQLFragment"
	ssgs.assertCases(
		NewSelectSQLGenerator("test", opts),
...
func (ssgs *selectSQLGeneratorSuite) TestGenerate_WithErroredBuilder() {
	...
	b := builder.NewSQLBuilder(true).SetError(errors.New("test error"))
	c := exp.NewSelectClauses().SetFrom(exp.NewColumnListExpression("test"))
	d.Generate(b, c)
	ssgs.assertErrorSQL(b, `pp: test error`)
}
...
func (ssgs *selectSQLGeneratorSuite) TestGenerate_withDistinct() {
	...
	expectedErr := "pp: dialect does not support DISTINCT ON clause [dialect=test]"
	ssgs.assertCases(
		NewSelectSQLGenerator("test", opts),
		selectTestCase{clause: sc, sql: `SELECT DISTINCT *`},
		selectTestCase{clause: sc, sql: `SELECT DISTINCT *`, isPrepared: true},
...
func (ssgs *selectSQLGeneratorSuite) TestGenerate_withJoin() {
	...
	expectedRjError := "pp: dialect does not support RightJoinType"
	expectedJoinCondError := "pp: join condition required for conditioned join LeftJoinType"
	ssgs.assertCases(
		NewSelectSQLGenerator("test", opts),
...
func (ssgs *selectSQLGeneratorSuite) TestToSelectSQL_withFor() {
	...
	scFnSlOf := sc.SetLock(exp.NewLock(exp.ForNolock, exp.SkipLocked, pp.T("my_table")))
	...
	scFsSlOf := sc.SetLock(exp.NewLock(exp.ForShare, exp.SkipLocked, pp.T("my_table")))
	scFsSlOfMulti := sc.SetLock(exp.NewLock(exp.ForShare, exp.SkipLocked, pp.T("my_table"), pp.T("table2")))
